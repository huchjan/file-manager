...
var useStdin = function () {
    ...
    if (input !== null) {
        ...
        if (inputSplit[0] == "cat") {
            ...
            findWord(inputSplit[1], inputSplit[2]);
        }
    }
};

function replaceWord(fileName, wordToReplace, replacement) {
    fs.readFile(fileName, function (err, data) {
        if (err) {
            console.log("Error");
        } else {
            var textData = data.toString();
            textData = textData.split(wordToReplace).join(replacement);
            fs.writeFile(fileName, textData, function (err) {
                if (err) {
                    console.log("Could not write to file");
                } else {
                    console.log("File successfully written in");
                }
            });
        }
    });
...
function findWord(fileName, wordToFind) {
    fs.readFile(fileName, function (err, data) {
        if (err) {
            console.log("Error");
        } else {
